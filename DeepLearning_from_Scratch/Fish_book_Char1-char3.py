...
from DeepLearning_from_Scratch.code.ch03.neuralnet_mnist import accuracy_cnt
...
# import torchvision
# train_set = torchvision.datasets.mnist.MNIST(root='./dataset', train=True, download=True)
# test_set = torchvision.datasets.mnist.MNIST(root='./dataset', train=False, download=True)
# print(train_set[0])
...
print(label)
print(img.shape)###(784,)
img = img.reshape(28,28)#####将array变为28*28的形式，也就是原来图片的格式。
img_show(img)

###神经网络的搭建。 2个隐藏层[50,100]，784个输入，10个输出
...
def init_network():
    with open("D:\Deeplearning\DeepLearning_from_Scratch\code\ch03\sample_weight.pkl",'rb') as f:
...
###这里为了实现predict函数一次性处理100张图片，可以将多张图片x[i]合并为一个数组：X，这样其结构由(784,) > (100,784)
###这样的处理叫批处理， 可以大幅度缩短每张图片的处理时间，优化运算时间，并减轻数据传送瓶颈。
###下述批处理代码:
...
for i in range(0,len(x),batch_size):###用法参考官方文档,range()左闭右开区间产生的一个数组
    x_batch = x[i:i+batch_size]
    Y_batch = predict(network,x_batch)##(100,10)的array
    p = np.argmax(Y_batch,axis=1)###(100,),axis表示argmax选取哪个维度进行分析，同时注意python 0 代表第一个，多以这个1是指第二个维度，也就是上面的（100，10）中的10，也可以理解为对于一个matrix，0维代表列方向，1维代表行方向
    print(p.shape)
    accuracy_cnt += np.sum(p == t[i:i+batch_size])###np.sum可以将bool > int
